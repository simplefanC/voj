...
import com.simplefanc.voj.utils.Tools;
    ...
    public static ProblemCrawler getCrawler(String remoteOj) {
        if (!crawlers.containsKey(remoteOj)) {
            synchronized (crawlers) {
                if (!crawlers.containsKey(remoteOj)) {
                    try {
                        List<Class<? extends ProblemCrawler>> crawlerClasses = Tools.findSubClasses("com.simplefanc.voj.crawler", ProblemCrawler.class);
