...
public class ConfigUtil {
    ...
    public String buildYamlStr(ConfigVo configVo) {
        return "voj:\n" + "  jwt:\n" + "    # 加密秘钥\n" + "    secret: " + configVo.getTokenSecret() + "\n"
                ...
                + UnicodeUtil.toUnicode(configVo.getBaseUrl(), false) + "\"\n" + "    name: \""
                + UnicodeUtil.toUnicode(configVo.getName(), false) + "\"\n" + "    short-name: \""
                + UnicodeUtil.toUnicode(configVo.getShortName(), false) + "\"\n" + "    description: \""
                + UnicodeUtil.toUnicode(configVo.getDescription(), false) + "\"\n" + "    register: "
                + configVo.getRegister() + "\n" + "    problem: " + configVo.getProblem() + "\n" + "    training: "
                + configVo.getTraining() + "\n" + "    contest: " + configVo.getContest() + "\n" + "    status: "
                + configVo.getStatus() + "\n" + "    rank: " + configVo.getRank() + "\n" + "    discussion: "
                + configVo.getDiscussion() + "\n" + "    introduction: " + configVo.getIntroduction() + "\n"
