        ...
        Runnable getResultTask = new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    Constants.Judge status = result.getStatusType() != null ? result.getStatusType() : Constants.Judge.STATUS_SYSTEM_ERROR;
                    if (status != Constants.Judge.STATUS_PENDING &&
                            status != Constants.Judge.STATUS_JUDGING &&
                            status != Constants.Judge.STATUS_COMPILING) {
                        ...
                        if (info.cid != 0) {
                            ...
                            if (status == Constants.Judge.STATUS_ACCEPTED) {
                                score = 100;
                            }
