...
Virtual Online Judge (VOJ) : 基于前后端分离、分布式架构的在线测评系统（VOJ），前端使用 Vue，后端主要使用 Spring Boot，Redis，MySQL，Nacos 等主流技术，**支持 HDU、POJ、MXT、JSK、TKOJ 的虚拟判题，同时适配手机端、电脑端浏览，拥有讨论区与站内消息系统，支持私有训练、公开训练（题单），还有完善的判题模式（普通测评、特殊测评、交互测评）和完善的比赛功能（打星队伍、关注队伍、外榜）。**
